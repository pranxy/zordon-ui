...
test('built Dropdown and Tooltip share one overlay bridge without duplicating its stack in component or primary bundles', async () => {
  const [dropdown, bridge, primary, declarations, manifest, tooltip, tooltipDeclarations] =
    await Promise.all(
      [
        ...
        'dist/components/fesm2022/pranxy-zordon-ui-tooltip.mjs',
        'dist/components/types/pranxy-zordon-ui-tooltip.d.ts',
      ].map(path => readFile(resolve(root, path), 'utf8')),
    );
  assert.match(dropdown, /from ['"]@pranxy\/zordon-ui\/internal-overlay['"]/);
  assert.match(tooltip, /from ['"]@pranxy\/zordon-ui\/internal-overlay['"]/);
  assert.doesNotMatch(tooltip, /class ZdOverlay(?:Coordinator|Stack)/);
  ...
  assert.ok(JSON.parse(manifest).exports['./tooltip']);
  assert.doesNotMatch(
    tooltipDeclarations,
    /(?:InputSignal|OutputEmitterRef)<[^;\n]*(?:OverlayRef|ZdOverlayHandle|ConnectedPosition)/,
  );
  assert.doesNotMatch(
    declarations,
