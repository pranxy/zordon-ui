...
| T06   | Add crawlability, performance, and release gates    | T02–T05     | Verified | CI route matrix and docs gates pass                                   | SSR/release 10/10; browser 13/13; a11y 1/1; links 7/7; 353.10 kB initial; review `Clear` |
...
| T06 | Primary agent + delegated release-gate lane | Shared workspace; metadata and test/tooling/config ownership split              | Production build; unit 9/9; SSR/release 10/10; browser 13/13; a11y 1/1; tooling 53/53; links 7/7; type/lint/format    | Independent implementation review: `Clear`                                           | No listeners remain on test ports 4310/4311 |
...
| T06 implementation      | Independent delegated reviewer                                                      | No material findings                                                        | Accept           | Final review: `Clear`                     |
...
| D09  | Limit structured data to truthful software and breadcrumb facts                                   | Canonical home emits `SoftwareApplication`; descendants emit `BreadcrumbList`; preview/404 omit JSON-LD   | Accepted |
| D10  | Build docs once in CI, then run separate SSR, browser, accessibility, link, and performance gates | Keeps failures attributable while avoiding repeated production builds; initial hard limit remains 400 KiB | Accepted |
